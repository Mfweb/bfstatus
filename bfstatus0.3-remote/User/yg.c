...
{
	if(KEY3 != KEY_DOWN)//如果按键3没按下 就进行标准检查
	{
		if(check)//如果需要检查EEPROM
		{
			if(read_ee() == 1)//如果EEPROM里面有数据
			{
				get_ratio();//计算比例
				return;//跳出
			}
	...
	Get_YG(0);//获得一次数据  此时认为摇杆在中间位置  油门在最低位置
	LED3 = OFF;
